...
type CNINetworkSuite struct {
	...
	*require.Assertions
	...
	network gardenbackend.Network
	cni     *gardenbackendfakes.FakeCNI
	store   *gardenbackendfakes.FakeFileStore
}

func (s *CNINetworkSuite) SetupTest() {
	var err error

	s.store = new(gardenbackendfakes.FakeFileStore)
	s.cni = new(gardenbackendfakes.FakeCNI)
	s.network, err = gardenbackend.NewCNINetwork(
		gardenbackend.WithCNIFileStore(s.store),
		gardenbackend.WithCNIClient(s.cni),
	)
	s.NoError(err)
}
...
func (s *CNINetworkSuite) TestNewCNINetworkWithInvalidConfigDoesntFail() {
	...
	// the plugins.
	//
	_, err := gardenbackend.NewCNINetwork(
		gardenbackend.WithCNINetworkConfig(gardenbackend.CNINetworkConfig{
...
func (s *CNINetworkSuite) TestSetupMountsCallsStoreWithNoNameServer() {
	network, err := gardenbackend.NewCNINetwork(
		gardenbackend.WithCNIFileStore(s.store),
	)
	s.NoError(err)
...
func (s *CNINetworkSuite) TestSetupMountsCallsStoreWithOneNameServer() {
	network, err := gardenbackend.NewCNINetwork(
		gardenbackend.WithCNIFileStore(s.store),
		gardenbackend.WithNameServers([]string{"6.6.7.7", "1.2.3.4"}),
	)
	s.NoError(err)
...
func (s *CNINetworkSuite) TestAddSetupErrors() {
	...
	task := new(containerdfakes.FakeTask)
