...
type ContainerSuite struct {
	...
	*require.Assertions
	...
	container           *gardenbackend.Container
	containerdContainer *containerdfakes.FakeContainer
	containerdProcess   *containerdfakes.FakeProcess
	containerdTask      *containerdfakes.FakeTask
	rootfsManager       *gardenbackendfakes.FakeRootfsManager
	killer              *gardenbackendfakes.FakeKiller
}

func (s *ContainerSuite) SetupTest() {
	s.containerdContainer = new(containerdfakes.FakeContainer)
	s.containerdProcess = new(containerdfakes.FakeProcess)
	s.containerdTask = new(containerdfakes.FakeTask)
	s.rootfsManager = new(gardenbackendfakes.FakeRootfsManager)
	s.killer = new(gardenbackendfakes.FakeKiller)

	s.container = gardenbackend.NewContainer(
...
func (s *ContainerSuite) TestPropertyNotFound() {
	...
	_, err := s.container.Property("any")
	s.Equal(gardenbackend.ErrNotFound("any"), err)
}
