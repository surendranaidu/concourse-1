...
func (s *RootfsManagerSuite) TestSetupCwdDirAlreadyExists() {
	mkdirCalled := false
	mgr := gardenbackend.NewRootfsManager(
		gardenbackend.WithMkdirAll(func(p string, mode os.FileMode) error {
			mkdirCalled = true
			return nil
		}),
	)
...
func (s *RootfsManagerSuite) TestSetupCwdWithoutIDMappings() {
	...
	mgr := gardenbackend.NewRootfsManager(
		gardenbackend.WithMkdirAll(func(p string, m os.FileMode) error {
			path = p
			mode = m
			return nil
		}),
