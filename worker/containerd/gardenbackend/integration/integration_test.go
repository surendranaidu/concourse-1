...
import (
	...
	containerdwrapper "github.com/concourse/concourse/worker/containerd"
...
type IntegrationSuite struct {
	...
	*require.Assertions
	...
	client            *containerdwrapper.Client
...
func (s *IntegrationSuite) SetupTest() {
	...
	s.backend, err = gardenbackend.New(
		containerdwrapper.NewClient(
			s.containerdSocket(),
			namespace,
			requestTimeout,
		),
...
func (s *IntegrationSuite) TestRunUnprivileged() {
	maxUid, maxGid, err := gardenbackend.NewUserNamespace().MaxValidIds()
	s.NoError(err)
...
func (s *IntegrationSuite) TestCustomDNS() {
	...
	network, err := gardenbackend.NewCNINetwork(
		gardenbackend.WithNameServers([]string{
			"1.1.1.1", "1.2.3.4",
		}),
	)
	s.NoError(err)

	networkOpt := gardenbackend.WithNetwork(network)
	customBackend, err := gardenbackend.New(
		containerdwrapper.NewClient(
			s.containerdSocket(),
			namespace,
			requestTimeout,
